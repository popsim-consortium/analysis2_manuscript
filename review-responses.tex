...
\end{point}
  ...
  \reply{
     ...
     We added two sections to the methods: one that provides detials on how DFE models are implemented using DFE objects and one describing how selective sweeps are modeled \revref.
     The first section contains a brief description of how the simulation engine uses the information specified in a given DFE object when sampling a new mutation.
     The second section describes how the conditioning for selective sweeps is implemented (indeed, by rejection sampling).
     We've also added brief clarification regarding the implementation of sweeps earlier in the paper, at \llname{sweep_conditioning}.
  }
...
\end{point}
  ...
  \reply{
    We have now included two additional figures showing the runtimes for both the whole genome simulations (used in demographic and DFE inference) and the 5Mb segment simulations of human chromosome 1   (used in the analyses of sweep detection) in Figures \ref{fig:sim-runtime} and \ref{fig:sim-sweeps-runtime}.
    With respect to the sweep simulations, we show that the simulation of sweeps in each of the segments takes approximately 3 minutes (Figure \ref{fig:sim-sweeps-runtime}).
    Given we simulated 1000 replicates of each one of the 100 segments, this means that the total runtime for the sweep simulations is approximately 300,000 minutes, or 5,000 hours.
    However, whole chromosome 1 simulations with BGS and sweeps would have been much more time consuming, each taking approximately 200 minutes (Figure \ref{fig:sim-runtime}),
    or in total, 20,000,000 minutes, or 333,333 hours). See \revref.
  }
  ...
  \reply{
    ...
    While it is possible to mix demographies and DFEs in \stdpopsim's catalog and study the effects, 
...
\begin{point}{\revref}
  ...
  \reply{
    In Fig. S1, $\pi$ is computed across all simulated genomes from all three populations,
...
\end{quote}
  ...
  \reply{
    We have added a note to the implementation section about how we handle overlapping annotations in a GFF file to \revref.
    In short, annotations can't overlap (we remove overlapping bits);
    see the documentation at \url{https://popsim-consortium.github.io/stdpopsim-docs/stable/api.html#stdpopsim.Contig.add_dfe}.
  }
...
Note that \grapes is also less accurate than \polydfe and \dadicli in inferring the shape of the distribution (Fig.~\ref{fig:homsap-dfe.ooa}B), which suggests the inaccuracy is not simply a scaling issue.
  ...
  we added error bars depicting the standard error to the mean values in Figure 3C (reflecting variation among the three replicate simulations).
  % 
...
\end{point}
 ...
 which cover 2.1\% of the human genome and 2.2\% of the vaquita genome.
 In each of these simulations, 70\% of exonic sites are assumed to be under selection (with gamma-distributed selection coefficients)
 and the remaining 30\% are assumed to be neutral.
