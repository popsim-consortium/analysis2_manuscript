...
\end{point}
  ...
  \reply{
    We have now included two additional figures showing the runtimes for both the whole genome simulations (used in demographic and DFE inference) and the 5Mb segment simulations of human chromosome 1 (used in the analyses of sweep detection) at \revref and \llname{sim-sweeps-runtime}.
    With respect to the sweep simulations, we show that the simulation of sweeps in each of the segments takes approximately 3 minutes \llname{sim-sweeps-runtime}.
    ...
    However, whole chromosome 1 simulations with BGS and sweeps would have been much more time consuming, each taking approximately 200 minutes \revref (or in total, 20,000,000 minutes, or 333,333 hours).
...
\end{quote}
  ...
  \reply{
    We have added a note to the implementation section about how we handle overlapping annotations in a GFF file.
    In short, we do not allow for overlapping annotations in a GFF file, and we prioritize the annotation being modeled (e.g., CDS) 
    over potentially overlapping annotations of other types (e.g., exons). 
    Further, we do not currently support multiple overlapping annotations of different types.
    \revref.
  }
