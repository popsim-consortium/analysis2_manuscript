...
\section*{Detection of Selective Sweeps}
    ...
    For each sweep location, we performed 2000 replicate simulations, sampling from them only if the selected allele reached a frequency of 0.95 or greater.
    ...
    \sweepfinder2 power remains more constant along the chromosome, while the $\pi$ cutoff method varies wildly. 
    ...
    These ROC curves, however, hide the previously seen phenomenon of high variation in power to detect sweeps along a realistic looking chromosome.
    ...
    % MFR: I changed this a bit: I agree with Dan that there is no clear pattern.
...
\section*{Methods}
    ...
    of (negtive) selection acting on exonic regions, matched to the human and vaquita porpoise genomes.
    ...
    (identified as OutOfAfrica\_3G09 in the stdpopsim library; \cite{gutenkunst_inferring_2009})
    and the genetic map estimated in \citet{frazer_second_2007} (HapMapII\_GRCh38).
    ...
    (ii) background selection, in which we applied the DFE estimated in \citet{kim_inference_2017} (Gamma\_K17) to exon annotations (taken from Ensembl; ensembl\_havana\_104\_exons),
