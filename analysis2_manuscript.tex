...
\section*{Introduction}
    ...
    non-equilibrium demographic histories, and where possible, modeling these features using estimates/annotations obtained
...
\section*{Implementing Selection in \stdpopsim}
    ...
    %    
    For example, say we wanted to implement a model of selection consistent with the
    ...
    with their proportions set to following their expected
    prevalence in exons.
    ...
    As with other aspects of \stdpopsim, we intend the selection models to be as biologicaly accurate as possible.
...
%    and the detection of selective sweeps, particularly in the context of realistic recombination maps and background selection.
    ...
    However, natural selection acting on linked sites (even when masked out of the analysis)
...
%    \end{figure}
    ...
    Reagrdless of these difference, we again see that background selection does not
...
\igcomment{I think that this is better conveyed now in the added histograms for $s$ in panel C}
   ...
   \kelcomment{I moved the last sentence from here to the Discussion and fleshed it out} 
    ...
    \begin{figure}[t!]
        ...
        Panel C shows the simulated distribution of $2 h s$ (gamma parameters averaged across replicates),
        along with the simulated distribution of $s$.
...
\section*{Methods}
    ...
    As described above, each method scales DFE slightly differently. 
    To make direct comparisons among them,
    we rescaled the selection coefficent for \grapes  as $s \mu / \theta$,
    \polydfe as $2 \mu s / \theta$, and
    \dadicli $\alpha * \nu / (\theta / (4 \mu L p))$.
    In each case, $\mu$ is the mutation rate,
    $\theta$ is each method's estimated value of the population scaled mutation rate ($\theta = 4N_{e}\mu$),
    $\alpha$ and $\nu$ are the inferred shape and rate parameters of the \dadicli's inferred gamma distribution, $L$ is the total number of sites available for the analysis,
