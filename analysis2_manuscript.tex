...
\section*{Abstract}
    ...
    has remained challenging, limiting our ability to benchmark statistical methods and explore theoretical predictions.
    ...
    and show how genomic features like recombination rate and functional density influence power to detect selective sweeps.
...
\section*{Introduction}
    ...
    and parameter scalings for their models, and may not be directly comparable.
...
\section*{Implementing Selection in \stdpopsim}
    ...
    \begin{wrapfigure}[37]{l}{0.4\textwidth}
        ...
        A schematic of the \stdpopsim catalog and the different components that
        can be used in simulations. The user specifies a species, a portion of
...
\section*{Example Applications with Selection}
    ...
    with and without background selection on exons. For this simulation, we used
    ...
    that acts on exonic positions, as defined from the HAVANA group release 104 for the human genome. 
    ...
    of simulations with a Gamma distributed DFE for the deleterious effects of mutations occurring exons.
...
\section*{Estimation of the Distribution of Fitness effects}
    ...
    the performance of different methods for inferring the DFE from genetic data. We performed simulations using the 
    ...
    % MFR: This is mentioning a constant scenario that is not currently shown here. Are we going to add it?
    ...
    With respect to the strength of selection, \polydfe seems slightly better than the other two methods.
...
\section*{Detection of Selective Sweeps}
    ...
    For each sweep location, we performed 2000 replicate simulations, sampling from them only if the selected allele reached a frequency of
    ...
    \begin{figure}
        ...
        True positive rate is shown for these methods for samples from the CEU
        and YRI samples (left and right respectively).  Under each method we
        show as a heat map exon density along the chromosome, along with the
        local recombination rate in cM/Mb.  Power is shown as the proportion of
        replicates in which a sweep was detected at each location.  To define
    ...
    more genetic variation, making sweeps more apparent. Further, the population size of YRI has been relatively constant over the last 100,000 years,
