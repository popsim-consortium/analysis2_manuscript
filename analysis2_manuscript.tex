...
\section*{Introduction}
    ...
    and genetic drift \citep[e.g.,][]{ohta1973slightly,lewontin1973distribution, kimura1985neutral, Tajima1989, gillespie1991causes, kern2018neutral}. \revpoint{2}{4}
    ...
    which provides an interesting comparison due to its small recent population size. \revpoint{1}{3}
...
\section*{Implementing Selection in \stdpopsim}
    ...
    namely, when a species of interest lacks a published DFE in the catalog \revpoint{1}{8}.
    ...
    For example, coding sequences may be associated with one DFE and non-coding sequences in exons with another DFE. \revpoint{2}{1}
    %%IG: I moved the comment about GFF overlaps here and edited because it fits here better than in the prev location.
    ...
    frequency at the beginning and end of the sweep period.  The conditioning \llabel{sweep_conditioning}
    ...
    we performed a similar analysis using the vaquita porpoise \textit{Phocoena sinus}. \llabel{vaquita_note}
...
\section*{Detection of Selective Sweeps}
    ...
    local exon density (Figure \ref{fig:power-exon}).  \revpoint{2}{6}
...
\section*{Discussion}
    ...
    benchmarking.
    Rather, we include these analyses as examples to highlight the utility of \stdpopsim \llabel{not_the_point}
    ...
    infer the DFE of $s$ along with $h$ (\textcite{wright1937distribution, veeramah2014evidence, simons2014deleterious, fuller2019measuring, balick2022overcoming, kyriazis2024constraining}; reviewed in \textcite{fuller2019measuring}) \revpoint{1}{6}
...
\section*{Methods}
    ...
    %DRS: I think this section looks great.
    ...
    % these may change.
    %
    %% For reference, from the docs on selective_sweep():
    ...
    % Simulation runtimes are shown in Figure \ref{fig:sim-runtime}. \revpoint{1}{4}
    ...
    which cover 2.1\% of the genome. \revpoint{2}{5}
    ...
    The running times of these simulations depend on chromosome length, the demographic model, and incorporation of selection (Figure \ref{fig:sim-runtime}). \revpoint{1}{4}
    ...
    which cover 2.2\% of the genome. \llabel{vaquita_percent}
    ...
    See Figure \ref{fig:sim-sweeps-runtime} for the distribution of runtimes for these simulations. \llabel{sim-sweeps-runtime}
...
\begin{figure}[h]
    ...
    Simulations were run on a mix of Intel and AMD machines (Intel Xeon Gold 6148 2.4 Ghz, Intel Xeon Gold 6248 2.5 Ghz, AMD EPYC 7513 2.6 Ghz and AMD EPYC 7713 2 Ghz).
    }
