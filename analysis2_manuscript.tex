...
\section*{Abstract}
    ...
    % BCH: "simulations... are challenging".  I would also reverse the order: "limiting our ability to explore... and to benchmark", because as
    % it is written not, "...and to explore" is very far from its first half "limiting our ability to...", making it hard for the reader to parse.
    ...
    % BCH: I'd put "DFEs", since it is plural, although I recognize that others might disagree...
    ...
    % BCH: "the density of functional sequence" does not seem grammatical to me; either "sequences" plural, or the fix I've recommended, which I think is better.
...
\section*{Introduction}
    ...
    requiring a parameterized demographic model,
    many choices including the strength and rate of selection,
    and implementation of genetic maps and other genomic features related to the strength of selection.
    % BCH: this reordering reads better to me since it was hard to tell whether "many choices" encompassed the parameterized demographic model or not;
    % perhaps a different way of restructuring it would be better, if I've distorted the intended meaning, such as "many choices regarding parameters
    % controlling selection and demography", e.g.
    This is a daunting task, and can be a barrier to
    the adoption of simulation-based methods in population genetics.
    Furthermore, different simulation and inference tools often use different structures % BCH: considered may -> might, and then decided "often" is more accurate!
...
\section*{Implementing Selection in \stdpopsim}
    % BCH: I mostly skimmed/skipped this section; if you'd find it useful for me to review it, let me know, I'm just a bit swamped.  :->
    \label{selection}
    ...
    \begin{figure}[t!]
        ...
        annotations. }
        % BCH: I notice that a rescaling factor of 20 is used here, which gives me pause.  I think this is my only major
        % comment on this manuscript: I wonder whether we ought to add a brief discussion of the dangers of rescaling,
        % specifically with respect to simulations involving selection since that is the topic at hand?  Dabi and Schrider
        % discuss this at some length in their recent paper.  I'm not suggesting rehashing everything they say, but rather,
        % adding one paragraph talking about the fact that stdpopsim offers a rescling parameter (as seen in this figure),
        % but that the use of it might be particularly problematic/unpredictable in simulations involving selection,
        % especially for large values of Q, and thus it ought to be used with caution, and with an effort to confirm that
        % the rescaling factor chosen is not substantially distorting the key results from the simulations, with a cite
        % to Dabi and Schrider.  (And probably also a cite to the other recent rescaling paper, which I forget the authors of.)
        % My impression from D&S is that Q=20 is quite high, and might well distort things significantly; so I wonder if we
        % also might want to show a smaller Q, like 2 or 5, in this figure so as not to give our imprimatur to people using Q=20!
    \end{figure}
...
\section*{Discussion}
    ...
    This performance is not surprising since decreasing $h$ for a given distribution of $s$ will lead to a decrease in $hs$, % "as" vs. "since" is a pet peeve :->
    ...
    A key distinction between our analyses and those done in practice % BCH: I don't like "in practice"; "those done in empirical work"?
    ...
    effectiveness of different detection methods using data that reflects the demographic history, recombination rate variation,
    ...
    methods that are well powered in the face of unknown demographic histories. Indeed, larger differences % BCH: not crazy about "well powered"; "with adequate statistical power even in the face of..."?
    ...
    on common ground in a way that was not possible before. % BCH: "on common ground" seems like an odd phrasing to me? "on a level playing field", perhaps?
