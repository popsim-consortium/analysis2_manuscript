...
\section*{Implementing Selection in \stdpopsim}
    ...
    % DRS: not editing this because I haven't looked at the implementation, but this doesn't seem right.
    % Obviously the allele must be allowed to drift outside of the bounds during the simulation, as long as it finishes
    % within those bounds. So if it is fixed during the sim when the bounds don't extend to 1.0 (or lost when the
    % bounds don't extent to 0.0) then we restart, and we restart if at the end of the sweep period we are outside of
    % the bounds. right? If what I am saying is correct then maybe this should be rephrased, and more importantly the
    % corresponding part of the methods should be corrected.
...
\section*{Estimation of the Distribution of Fitness effects}
    ...
    \begin{figure}[b!]
        ...
        (C) Binned distribution of $s$ implied by the average DFE inferred from the three simulated replicates (averaging the inferred gamma parameters);
...
\section*{Discussion}
    ...
    benchmarking. Rather, we include these analyses as examples to highlight the utility of \stdpopsim
    ...
    while a gamma distribution might fit the conditional distribution of $s$ given $h$, a relationship between $h$ and $s$ might result in a magrinal distribution for $hs$ which is not well approximated by a gamma distribution.
...
\section*{Methods}
    ...
    % DRS: see my earlier comment about how we implement the conditioning here
    ...
    Thus, the mean value of $s$ in \slim terms is obtained by  $\frac{4\mu}{\theta} \alpha \nu L p$.
